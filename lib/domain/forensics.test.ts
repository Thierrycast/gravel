...
describe("findHiddenSubscriptions", () => {
  ...
  it("exclui Pix enviado recorrente da triagem de assinatura", () => {
    const result = findHiddenSubscriptions([
      tx({ date: "2026-01-10", amount: -1780, description: "Pix enviado - Ana Paula De Souza Lima" }),
      tx({ date: "2026-02-10", amount: -1782.33, description: "Pix enviado - Ana Paula De Souza Lima" }),
      tx({ date: "2026-03-10", amount: -1784, description: "Pix enviado - Ana Paula De Souza Lima" }),
    ])
    expect(result).toEqual([])
  })
