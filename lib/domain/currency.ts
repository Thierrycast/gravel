...
export function sumConvertedToBrl<T>(
  ...
  const rate = new Prisma.Decimal(usdBrlRate);
  return rows.reduce((total, row) => {
    const amount = decimal(amountOf(row));
    const code = normalizeCurrencyCode(currencyOf(row));

    if (BRL_CODES.has(code)) {
      return total.plus(amount);
    }
    if (USD_CODES.has(code)) {
      return total.plus(amount.mul(rate));
    }
    // Fallback or other currencies could be added here
    return total.plus(amount);
  }, new Prisma.Decimal(0));
}
