 ...
 */
export type VoicePalette = Partial<Record<MotionState, [number, number, number]>>;

/**
 * A cor de cada estado. O default abaixo veio da Vela; `setVoicePalette` troca o que você quiser
 * sem tocar no resto.
 *
 * Atenção ao adaptar: isto não é decoração, é a informação mais importante da tela. Quem está
 * falando — você ou a assistente — precisa estar em **extremos opostos de temperatura**: azul frio
 * em `listening`, âmbar quente em `speaking`. Matizes vizinhos (dois azuis, dois roxos) fazem o
 * visual parar de informar e voltar a ser só enfeite bonito. Troque a família de cor à vontade;
 * mantenha o contraste de temperatura entre esses dois estados.
 */
...
/** Mescla a paleta do seu app sobre o default. Tema é global, não estado de instância. */
export function setVoicePalette(palette: VoicePalette) {
  for (const [state, cor] of Object.entries(palette)) {
    if (cor) STATE_MOOD[state as MotionState] = cor;
  }
}
